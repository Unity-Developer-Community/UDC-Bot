...
| **Conversion** | Temperature (`/ftoc`, `/ctof`) and currency conversion (`/curr` with code autocomplete, plus the `!curr` text command) | `ConvertSlashModule`, `ConvertModule` | `CurrencyService` | Feature |
