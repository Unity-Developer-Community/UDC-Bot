...
public sealed class RecruitmentOwnerActionTests
{
    ...
    public async Task WrongOwnerGuildThreadAndGeneration_AreRejectedBeforePublicMutation()
    {
        ...
        foreach (var context in new[] { f.Owner with { UserId = 999 }, f.Owner with { GuildId = 2 }, f.Owner with { ThreadId = 11 } })
            await Assert.ThrowsAsync<InvalidOperationException>(() => f.Owners.PrepareAsync(context, "0123456789ABCDEF", RecruitmentActionKind.Delete, default));
        await Assert.ThrowsAsync<InvalidOperationException>(() => f.Owners.PrepareAsync(f.Owner, "0123456789ABCDEF", RecruitmentActionKind.Delete, default));
    ...
    {
        await using var f = new RecruitmentAdvisoryFixture(); await f.StartAsync();
        var confirmation = await f.Owners.PrepareAsync(f.Owner, await f.Generation(), RecruitmentActionKind.Delete, default);
        if (change == "expired") f.Time.Advance(TimeSpan.FromMinutes(3));
        if (change == "accepted") await f.Store.UpdateAsync(state => { state.Posts[10].AcceptedAtUtc = f.Time.Now; state.Posts[10].Acknowledgement = RecruitmentAcknowledgement.Passed; return true; });
    ...
    [TestMethod]
    [DataRow(RecruitmentActionKind.Delete)]
    [DataRow(RecruitmentActionKind.LockArchive)]
    public async Task LostActionResponse_IsRecoveredWithoutRepeatingMutation_AndReplayIsRejected(RecruitmentActionKind kind)
    {
        await using var f = new RecruitmentAdvisoryFixture(); await f.StartAsync();
        if (kind == RecruitmentActionKind.LockArchive)
    ...
    public async Task ExplicitRemoval_PreservesPriorAcceptedDeletionHistory()
    {
        ...
        Assert.AreEqual(f.Time.Now, state.Authors[123].Groups[RecruitmentListingGroup.Recruiting].LastAcceptedDeletedAtUtc);
    }
