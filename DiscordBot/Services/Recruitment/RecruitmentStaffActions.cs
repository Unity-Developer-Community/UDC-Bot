...
public enum RecruitmentResponseReview { KeepUnknown, NoQualifyingResponses, QualifyingResponsePresent }

/// <summary>Authenticated staff commands call mutations under the managed work gate; every change retains a reason.</summary>
public sealed class RecruitmentStaffActions(RecruitmentStateStore store, IRecruitmentPublisher discord,
    ...
    IOptions<RecruitmentOptions> options, TimeProvider time)
{
    ...
    public async Task<string> StatusAsync(ulong threadId, CancellationToken token)
    {
        ...
        var eligibility = new RecruitmentPolicyEvaluator(options.Value, time).EvaluateEligibility(state, threadId);
    ...
    public async Task RestartAcknowledgementAsync(ulong threadId, ulong actorId, string reason, CancellationToken token)
    {
        ...
        if (post.AcceptedAtUtc is not null || post.Lifecycle != RecruitmentLifecycle.Open || post.ClosedRequested || post.IsExempt)
    ...
    public Task WaiveCooldownAsync(ulong threadId, ulong actorId, string reason, CancellationToken token)
    {
        ...
        return store.UpdateAsync(state =>
        {
            ...
            foreach (var accepted in state.Posts.Values.Where(previous => previous.AuthorId == post.AuthorId &&
                         previous.AcceptedAtUtc is not null && RecruitmentForumClassifier.GroupOf(previous.Forum) == RecruitmentForumClassifier.GroupOf(post.Forum)))
            {
                created = RecruitmentHistory.Later(created, accepted.CreatedAtUtc);
                deleted = RecruitmentHistory.Later(deleted, accepted.DeletedObservedAtUtc);
            }
            ...
            Record(state, post, actorId, "Cooldown waived for " + RecruitmentForumClassifier.GroupOf(post.Forum), reason);
            return true;
        }, token);
    }
    ...
    public async Task ReviewAsync(ulong threadId, RecruitmentResponseReview responseReview, bool dismiss,
        ulong actorId, string reason, CancellationToken token)
    {
        ...
        await store.UpdateAsync(state =>
        {
            ...
            if (responseReview != RecruitmentResponseReview.KeepUnknown)
            {
                if (responseReview == RecruitmentResponseReview.NoQualifyingResponses && saved.FirstQualifyingResponseAtUtc is not null)
                ...
                if (responseReview == RecruitmentResponseReview.QualifyingResponsePresent)
    ...
    public async Task AdoptAsync(ulong threadId, ulong actorId, string reason, CancellationToken token)
    {
        ...
        if (post.Lifecycle != RecruitmentLifecycle.Open || post.ClosedRequested || post.IsExempt || live.Pinned || live.OrdinaryAuthor != true ||
        ...
        await store.UpdateAsync(state =>
        {
            ...
            if (!new RecruitmentPolicyEvaluator(options.Value, time).TryAccept(state, threadId).IsEligible)
    ...
    public async Task ResolveMissingAsync(ulong threadId, DateTimeOffset deletedAtUtc, ulong actorId, string reason, CancellationToken token)
    {
        ...
        if (post.Lifecycle != RecruitmentLifecycle.Missing || await discord.GetPostAsync(threadId, token) is not null ||
    ...
    public async Task ChangeLifecycleAsync(ulong threadId, RecruitmentActionKind kind, ulong actorId, string reason, CancellationToken token)
    {
        RequireReason(reason);
        if (kind is not (RecruitmentActionKind.LockArchive or RecruitmentActionKind.Reopen))
            throw new InvalidOperationException("Deletion requires its separate confirmation.");
        var state = (await store.LoadAsync(token))!;
        var post = state.Posts[threadId];
        await RequireLiveAsync(post, token);
        if (kind == RecruitmentActionKind.Reopen)
        {
            if (post.Lifecycle != RecruitmentLifecycle.Closed) throw new InvalidOperationException("Only a closed listing can be reopened.");
            if (state.Posts.Values.Any(other => other.ThreadId != threadId && other.AuthorId == post.AuthorId &&
                other.AcceptedAtUtc is not null && other.Lifecycle == RecruitmentLifecycle.Open &&
                RecruitmentForumClassifier.GroupOf(other.Forum) == RecruitmentForumClassifier.GroupOf(post.Forum)))
                throw new InvalidOperationException("Another accepted listing occupies this group; close it before reopening.");
        }
        await lifecycle.RequestAsync(threadId, kind, RecruitmentActionOrigin.Moderator, RecruitmentCloseReason.Moderator, actorId, reason, token);
        await lifecycle.RecoverAsync(threadId, token);
    }

    public async Task<RecruitmentConfirmation> PrepareRemovalAsync(ulong threadId, ulong actorId, string reason, CancellationToken token)
    {
        RequireReason(reason);
        var post = (await store.LoadAsync(token))!.Posts[threadId];
        await RequireLiveAsync(post, token);
        ...
        {
            var saved = state.Posts[threadId];
            var confirmation = new RecruitmentConfirmation { Token = RecruitmentGuidelines.NewToken(), Kind = RecruitmentActionKind.Delete,
                ActorId = actorId, Origin = RecruitmentActionOrigin.Moderator, Note = reason, Version = saved.Advisory.Version,
    ...
    public async Task ConfirmRemovalAsync(ulong threadId, ulong actorId, string tokenValue, CancellationToken token)
    {
        ...
        if (confirmation is null || confirmation.Token != tokenValue || confirmation.ActorId != actorId ||
            confirmation.Origin != RecruitmentActionOrigin.Moderator || confirmation.ExpiresAtUtc <= Now ||
        ...
        await RequireLiveAsync(post, token);
        await lifecycle.RequestAsync(threadId, RecruitmentActionKind.Delete, RecruitmentActionOrigin.Moderator,
            RecruitmentCloseReason.Moderator, actorId, confirmation.Note, token, confirmation.Token);
        await lifecycle.RecoverAsync(threadId, token);
    }

    private async Task<RecruitmentPublicPost> RequireLiveAsync(RecruitmentPostRecord post, CancellationToken token)
    {
    ...
    private void Record(RecruitmentStateDocument state, RecruitmentPostRecord post, ulong actorId, string operation, string reason)
    {
        post.Audit.Add(new(RecruitmentGuidelines.NewToken(), Now, actorId, operation, reason));
        ...
        RecruitmentHistory.Author(state, post).LastActivityAtUtc = Now;
    }

    private static void ClearResolvedGroupHold(RecruitmentStateDocument state, RecruitmentPostRecord post)
    {
        if (!state.Posts.Values.Any(other => other.AuthorId == post.AuthorId &&
            RecruitmentForumClassifier.GroupOf(other.Forum) == RecruitmentForumClassifier.GroupOf(post.Forum) &&
            (other.RequiresReview || other.DeletionTimeUncertain || other.Lifecycle == RecruitmentLifecycle.Missing)))
            RecruitmentHistory.Group(RecruitmentHistory.Author(state, post), post.Forum).RequiresReview = false;
    }
