...
public class ReminderModule : ModuleBase
{
    ...
    public async Task RemindMe(string time, [Remainder] string message)
    {
        ...
        var reminderDate = global::DiscordBot.Utils.Utils.ParseTimeFromString(time);
...
"Invalid format for reminder.\\nCorrect Syntax: ``!remindme <1hour5m> <optional message>``")
        ...
        var formattedTime = global::DiscordBot.Utils.Utils.FormatTime((uint)(reminderDate - DateTime.Now).TotalSeconds);
        await (ReplyAsync(
    ...
    public async Task Reminders(IUser user)
    {
        ...
        {
            var msgLink = global::DiscordBot.Utils.Utils.MessageLinkBack(Context.Guild.Id, reminder.ChannelId, reminder.MessageId);
            var timeLeft = global::DiscordBot.Utils.Utils.FormatTime((uint)(reminder.When - DateTime.Now).TotalSeconds);
            embed.AddField(
                $"#{index++} | {timeLeft}",
                $"[Link]({msgLink}) \"{reminder.Message}\"");
        }
        if (await Context.Guild.GetChannelAsync(Settings.Channels.BotCommands.Id) is IMessageChannel botCommands)
            await (botCommands
